...
// Form submit handler with user info
...
function fetchExpenseSummary() {
  fetch(`/api/expenses/summary?username=${user.username}`)
    .then(res => res.ok ? res.json() : Promise.reject('Summary fetch failed'))
    .then(data => {
...
// Fetch expenses and show category folders
...
// Show expenses for a chosen category
